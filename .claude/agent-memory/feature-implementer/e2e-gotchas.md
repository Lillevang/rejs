...
**Why:** Each cost a full debug cycle. **How to apply:** before declaring done, ensure no manual dev server lingers and keep `workers: 1`; when asserting on a saved/loaded plan name scope the selector; for any map-drag test reach for `dragTo`+poll, never raw mouse events. See also [[testing-setup]].
