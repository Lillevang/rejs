...
- **`resolve()` stays pure and produces NO diagnostics.** Parse diagnostics come from `parse.ts`; resolution-time warnings (date sanity) come from a _separate_ exported pure fn `dateDiagnostics(parsed, resolved)` in `resolve.ts`. `App` merges `[...parseDiagnostics, ...dateDiagnostics(trip, resolved)]` sorted by line and feeds the single list to both `Editor` (error/warning badge counts) and `Diagnostics.tsx` (which already styles `--warning` distinctly). So a new resolution-time check = add to `dateDiagnostics`, no plumbing changes. `Severity` ("error" | "warning") already exists in `types.ts`. Pattern for date warnings: only fire when the user _explicitly_ wrote the dates (check `parsed.hops[i].startDate`, since `resolve` overwrites `hop.startDate` with the resolved value) — chained `stay:` hops abut by construction, so flagging them is noise; undated plans must produce zero warnings.
